...
// Blit to screen
bool ModuleRender::Blit(SDL_Texture* texture, int x, int y, SDL_Rect* section, float speed, double angle, bool flip, bool use_camera, int pivot_x, int pivot_y)
{
	bool ret = true;
	SDL_Rect rect;
	rect.x = (int) (camera.x * ((use_camera) ? speed : 0)) + x * SCREEN_SIZE;
	rect.y = (int) (camera.y * ((use_camera) ? speed : 0)) + y * SCREEN_SIZE;
	...
	if(flip == false && SDL_RenderCopyEx(renderer, texture, section, &rect, angle, p, SDL_FLIP_NONE) != 0)
	{
	...
	{
		SDL_RenderCopyEx(renderer, texture, section, &rect, angle, p, SDL_FLIP_HORIZONTAL);
	}
...
void ModuleRender::CameraFollow(iPoint p) {
	...
	camera.y = CLAMP(-p.y + SCREEN_HEIGHT / 2, -1322 + SCREEN_HEIGHT, 0);
}